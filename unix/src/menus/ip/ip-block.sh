...
checkCurlInstalled() {
	# Using the "hash" command to check if the "curl" command exists".
	# Depending on the exit code, the "runnable" variable is either incremented or not.
...
updateBlacklist() {
	...
	# The standard output of cURL command, used to download the blacklist is appended to 
	# the log file.
...
blockFromBlacklist() {
	...
	# Based on the output of "ip route", the file shown to either be "already blocked" or 
	# "blocked successfully".
	# The phrases "already blocked" and "blocked successfully" are colors used ANSI 
	# Escape Sequences.
...
# Checking for dependencies and using the "runnable" variable to make sure that the script 
# doesn't run unless they are installed.
checkCurlInstalled
